 ...
 * It extends the parent class "Request", where it presents the requests of type
 * POSt.
...
PostRequest::PostRequest(std::string request,
    ...
    : Request(Type::POST), headers_(headers) {
...
/**
 * Process a POST request by sending 200 OK resposne to the client and then
 * waiting for the file to be sent back to the server and write it to the
 * specified directory.
 * @param socket Socket used to process the request from.
 */
void PostRequest::process(Socket *socket) {
    std::string response = build_response(Request::SUCCESS_RESPONSE_STATUS,
                                          Request::HTML_MIME_TYPE, "");
    socket->writeOutput(response);
    std::string file_content =
        socket->readInput(std::stoi(headers_[Request::CONTENT_LENGTH_HEADER]));
